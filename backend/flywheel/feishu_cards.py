...
def build_novel_test_feishu_card(payload: dict[str, object]) -> dict[str, object]:
    report = payload.get("report_zh") if isinstance(payload.get("report_zh"), dict) else {}
    platform = str(report.get("目标平台") or payload.get("publish_platform") or "全部平台").strip() or "全部平台"
    title = "小说测试报告"
    generated_at = str(report.get("生成时间") or payload.get("generated_at") or "").strip()
    report_day = _day_label(generated_at or payload.get("date") or "")
    requested = int(report.get("计划小说数") or report.get("请求数量") or 0)
    generated = int(report.get("已生成") or 0)
    published = int(report.get("已发布") or 0)
    failed = int(report.get("失败") or 0)
    overview_rows = _feishu_pairs_table_rows(
        [
            ["生成时间", generated_at],
            ["目标平台", platform],
            ["执行模式", str(report.get("执行模式") or payload.get("mode") or "小说发布")],
            ["计划条数", f"{requested} 条"],
            ["已生成", f"{generated} 条"],
            ["已发布", f"{published} 条"],
            ["失败", f"{failed} 条"],
            ["环境", str(report.get("环境") or payload.get("environment_zh") or "-")],
        ]
    )
    detail_rows = [
        [
            item.get("序号") or item.get("index") or index,
            item.get("账号") or item.get("account") or "-",
            item.get("小说") or item.get("title") or "-",
            item.get("生成链路") or item.get("generation_chain") or "-",
            item.get("段数") or item.get("segment_count") or "-",
            item.get("视频时长") or item.get("total_duration") or "-",
            item.get("发布状态") or item.get("publish_status_zh") or item.get("publish_status") or item.get("status") or "-",
            item.get("失败原因") or item.get("failure_reason") or item.get("error") or "-",
        ]
        for index, item in enumerate((report.get("任务明细") if isinstance(report.get("任务明细"), list) else []), start=1)
    ]
    key_points = []
    if requested > 0:
        key_points.append(f"本轮共 {requested} 条小说任务，已生成 {generated} 条，已发布 {published} 条，失败 {failed} 条。")
    summary = str(payload.get("user_summary_zh") or "").strip()
    if summary:
        key_points.extend([line.strip() for line in summary.splitlines() if line.strip()][:3])
    elements: list[dict[str, object]] = [
        _feishu_markdown_block(
            f"{title}已完成；平台 **{platform}**，报告日期 {report_day}，已生成 **{generated}** 条，已发布 **{published}** 条。"
        ),
        _feishu_rule(),
    ]
    elements.extend(
        _table_section(
            ...
            [[a, b, c, d] for a, b, c, d in overview_rows],
            page_size=10,
            header_background_style="orange",
        )
    )
    elements.extend(
        _table_section(
            ...
            [
                ...
                ("account", "账号", "text", "auto"),
                ("novel", "小说", "text", "auto"),
                ("chain", "生成链路", "text", "auto"),
                ("segments", "段数", "text", "auto"),
                ("duration", "视频时长", "text", "auto"),
                ("status", "发布状态", "text", "auto"),
                ("remark", "备注", "text", "auto"),
            ],
    ...
    if key_points:
        elements.extend(_bullet_block("三、执行总结", key_points, limit=5))
    return {
        "config": {"wide_screen_mode": True, "width_mode": "fill", "enable_forward": True},
        "header": {
            "template": "orange",
