...
def _stardusttv_title_file_path() -> Path:
    raw = str(os.getenv("BARRY_LOOP_STARDUSTTV_FILE") or "").strip()
    if raw:
        return Path(raw).expanduser().resolve()
    return (MODULE_ROOT_DIR / "data" / "stardusttv_titles.json").resolve()


def _stardusttv_cursor_state_path() -> Path:
    raw = str(os.getenv("BARRY_LOOP_STARDUSTTV_STATE_FILE") or "").strip()
    if raw:
        return Path(raw).expanduser().resolve()
    state_root = str(os.getenv("BARRY_LOOP_STATE_ROOT") or "").strip()
    base = Path(state_root).expanduser().resolve() if state_root else (MODULE_ROOT_DIR / "runtime" / "continuous-loop").resolve()
    return (base / "_shared" / "stardusttv_cursor.json").resolve()


def _load_stardusttv_titles_from_excel(path: Path) -> list[dict[str, object]]:
    try:
        from openpyxl import load_workbook
    except Exception:
        return []
    ...
    app_idx = header.index("app_id") if "app_id" in header else 0
    serial_idx = header.index("serial_id") if "serial_id" in header else 1
    title_idx = header.index("serial_name") if "serial_name" in header else (header.index("剧名") if "剧名" in header else 2)
    rows: list[dict[str, object]] = []
    seen: set[str] = set()
    for row_number, row in enumerate(worksheet.iter_rows(min_row=2, values_only=True), start=2):
        ...
        if not title or title in seen:
            continue
        seen.add(title)
        app_id = str(row[app_idx] or "stardusttv").strip().lower() if app_idx < len(row) else "stardusttv"
        serial_id = str(row[serial_idx] or "").strip() if serial_idx < len(row) else ""
        rows.append(
            {
                "row_number": row_number,
                "app_id": app_id or "stardusttv",
                "serial_id": serial_id,
                "title": title,
            }
        )
...
def _load_stardusttv_titles_from_json(path: Path) -> list[dict[str, object]]:
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except Exception:
        return []
    ...
    for index, item in enumerate(payload, start=1):
        if not isinstance(item, dict):
            continue
        title = str(item.get("title") or item.get("serial_name") or item.get("剧名") or "").strip()
        if not title or title in seen:
            continue
        seen.add(title)
        rows.append(
            {
                "row_number": int(item.get("row_number") or index),
                "app_id": str(item.get("app_id") or "stardusttv").strip().lower() or "stardusttv",
...
def _load_stardusttv_titles() -> list[dict[str, object]]:
    path = _stardusttv_title_file_path()
    if not path.exists():
        return []
    if path.suffix.lower() in {".xlsx", ".xlsm", ".xltx", ".xltm"}:
        return _load_stardusttv_titles_from_excel(path)
    return _load_stardusttv_titles_from_json(path)


def _load_stardusttv_cursor_state() -> dict[str, object]:
    path = _stardusttv_cursor_state_path()
    if not path.exists():
        return {"next_index": 0, "cycle_count": 0}
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
...
def _save_stardusttv_cursor_state(next_index: int, cycle_count: int) -> None:
    path = _stardusttv_cursor_state_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "next_index": max(0, int(next_index or 0)),
...
def _select_stardusttv_sources(args: argparse.Namespace, config, *, target_count: int | None = None) -> tuple[list[dict], list[dict]]:
    del config
    desired_count = max(args.count, int(target_count or args.count))
    rows = _load_stardusttv_titles()
    if not rows:
        raise SystemExit(f"StardustTV 剧名表为空或缺失：{_stardusttv_title_file_path()}")

    state = _load_stardusttv_cursor_state()
    total_rows = len(rows)
    cursor = int(state.get("next_index") or 0) % total_rows
    cycle_count = int(state.get("cycle_count") or 0)
    retry_count = max(0, int(getattr(args, "episode_probe_retry_count", 0) or 0))
    ...
    while len(sources) < desired_count and scanned < max_scan:
        row = dict(rows[cursor])
        title = str(row.get("title") or "").strip()
        app_id = str(row.get("app_id") or "stardusttv").strip().lower() or "stardusttv"
        drama, search_meta = _find_official_title_match(title, app_id)
        _emit_status_line(
            "stardusttv_title_search",
            {
                "line": _line_name(),
                **search_meta,
                "row_number": int(row.get("row_number") or 0),
                "source_serial_id": str(row.get("serial_id") or ""),
                "matched": bool(drama),
            },
        )
        if not drama:
            ...
            if not bool(episode.get("supported")):
                ...
                drama["source_mode"] = "official_ffmpeg"
                drama["candidate_fetch_source"] = "stardusttv_official_ffmpeg"
                drama["stardusttv_row_number"] = int(row.get("row_number") or 0)
                drama["stardusttv_source_serial_id"] = str(row.get("serial_id") or "")
                drama["stardusttv_cycle_count"] = cycle_count
                sources.append({"drama": drama, "episode": episode})
        scanned += 1
        cursor = (cursor + 1) % total_rows
        if cursor == 0:
    ...
    if bool(getattr(args, "execute", False)):
        _save_stardusttv_cursor_state(cursor, cycle_count)
    if not sources:
        reason_counts = _count_skip_reasons(skipped)
        _emit_status_line(
            "stardusttv_selection_empty",
            {
                "line": _line_name(),
                "title_count": total_rows,
                "matched_source_count": 0,
...
def _build_batch_plan(args: argparse.Namespace, config) -> dict[str, object]:
    ...
    elif _recent_order_mode_enabled():
...
def _publish_batch_item(item: dict, args: argparse.Namespace, platform: str, attempt: int) -> dict:
    ...
    try:
        ...
        payload = {
            "team_id": str(account.get("team_id") or ""),
            "title": str(drama.get("title") or item.get("title") or "").strip(),
