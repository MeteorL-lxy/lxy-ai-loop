...
5. If the user asks for random drama selection, drama-library selection, automatic drama posting, or statistics in natural language, use the automatic flywheel flow even if the user never says "barry-video" or "飞轮". Trigger phrases include "随机选剧", "随机挑剧", "从短剧库选", "剧你随机选", "自动选剧", "自动发布", "跑一轮", "发一轮", "统计数据", "每个账号发一条", "分别发布", "批量发短剧", "20 个账号发 20 条". For one drama/account use `run-round` or `barry_video_flywheel_round`; for multiple dramas, multiple accounts, a requested count, or "每个账号一条", use `barry_video_batch_drama` or `run-batch-drama`. Before any real publish, the platform and target accounts must be resolved.
   Important Facebook short-drama default:
   - If the request is for short-drama batch publishing to Facebook and the user gives a count such as "10 个 fb 账号", "我的 10 个 Facebook 账号", "发布到 10 个 FB 账号", or "每个账号一条", do not ask which specific Facebook accounts to use.
   - Facebook 短剧没有统一默认主池。必须使用当前已拆分的执行池之一，或由用户明确指定目标池/账号。
   - Do not stop to ask "随机选哪 10 个账号" vs "指定哪 10 个账号" for this case.
...
- Publish target confirmation is shared by all flows. Do not infer TikTok/Facebook/account defaults from config or previous examples. For Facebook short-drama requests, do not assume a unified default pool; use the explicitly configured execution pool for that line, or wait for the user's pool/account selection when the target is ambiguous. If the request itself resolves the target set, such as "选择 20 部剧发布到 20 个 FB 账号", proceed directly with those explicitly resolved accounts instead of stopping for another confirmation.
