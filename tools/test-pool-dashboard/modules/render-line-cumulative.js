      ...
      <table class="data-table compact line-cumulative-table">
        <thead>
          <tr>
            ...
            <th>累计播放</th>
            <th>累计点击</th>
            <th>累计收益明细<br><span class="table-sub">分佣 / 广告 / 订单</span></th>
            <th>累计互动</th>
          </tr>
        </thead>
        <tbody>
          ${rows.length ? rows.map((row) => `
            <tr>
              ...
              <td class="income-breakdown">
                <span>分佣 ${fmtMoney(row.share_income_total)}</span>
                <span>广告 ${fmtMoney(row.ad_income_total)}</span>
                <span>订单 ${fmtMoney(row.order_amount_total)}</span>
              </td>
